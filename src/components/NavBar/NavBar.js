...
function NavBar({ loggedIn }) {
  ...
  return (
    <nav className={loggedIn ? 'NavBar__container' : 'NavBar__container NavBar__container_logout'}>
      {loggedIn ?
        (<>
        ...
        :
          (<>
            ...
            <Link to='/signin' onClick={linkRouteAuthorization} className="NavBar__link_startScreen NavBar__link_startScreen_auth">{textBarAuthorization}</Link>
          </>)
      }
    </nav>
  );
